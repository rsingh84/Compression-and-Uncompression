...
int FancyInputStream::read_int() {
    ...
    input_file.read((char*)&num, sizeof(int)); // read the number and store it in 'num'
    if(input_file.eof()) {                     // not enough bytes in the file to read it
        error("Not enough bytes to read the next int");
    }
    return num;
}
int FancyInputStream::read_threebyte() {
    if(buffer_index != 8) {
        error("Attempting to read int when bitwise buffer is not empty");
    }
    unsigned int num;                          // temporary variable to store the number
    input_file.read((char*)&num, 3*sizeof(char)); // read the number and store it in 'num'
    if(input_file.eof()) {                     // not enough bytes in the file to read it
        error("Not enough bytes to read the next int");
    }
    return num;
...
void FancyOutputStream::write_int(int const & num) {
    ...
    output_file.write((char*)&num, sizeof(int)); // write 'num' to file
}
void FancyOutputStream::write_threebyte(int const & num) {
    if(buffer_index != 0) {
        error("Attempting to write int when bitwise buffer is not empty");
    }
    output_file.write((char*)&num, 3*sizeof(char)); // write 'num' to file
}
